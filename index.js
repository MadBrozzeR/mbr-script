...
(function(mbr) {
  ...
  function assign () {
    ...
    for (var index = 1 ; index < arguments.length ; ++index) {
      for (var key in arguments[index]) {
        result[key] = arguments[index][key];
      }
    }
  ...
  Ajax.prototype.getUrl = function () {
    ...
    if (this.options.params) {
      for (var key in this.options.params) if (this.options.params[key] !== undefined) {
  ...
  mbr.router = function (routes, params) {
    ...
    var router = {
      ...
      attach: function (attachWindow) {
        ...
        var current = parseHash(win.location.hash);
        router.last = {
          hash: current.path,
      ...
      go: function (hash, params) {
        router.last = {hash: hash, params: params};
        win.location.hash = CONST.SYMBOL.HASH + hash + serializeHashParams(params);
      },
      apply: function (hash, params) {
        if (hash === router.last.hash) {
          router.go(hash, assign({}, router.last.params, params));
        } else {
          router.go(hash, params);
        }
      }
    };
    return router;
  ...
  function Form (props) {
    ...
    this.onSubmit = props.onSubmit || on.submit;
    this.onResponse = props.onResponse || on.response;
    this.method = props.method || CONST.METHOD.POST;
  ...
  Form.prototype.attachTo = function (block) {
    ...
    mbr.event(block, CONST.EVENT.KEYPRESS, function (event) {
      if (event.which === 13 && !(event.target instanceof HTMLTextAreaElement && !event.ctrlKey)) {
        _this.submit();
      }
    });
  }
  ...
  ClassName.prototype.switch = function (value) {
    this.switcherValue && this.del(this.switcherValue);
    this.switcherValue = value;
    this.add(value);
  }
  ClassName.prototype.switcher = function (switcher) {
    this.switcherObject = switcher;
    return this;
  }
  ClassName.prototype.switchTo = function () {
    var switcher = this.switcherObject;
    if (switcher) {
      if (switcher.current === this) {
        return false;
      }
      if (switcher.current) {
        switcher.current.del(switcher.modifier);
      }
      this.add(switcher.modifier);
      switcher.current = this;
      return true;
    } else {
      console.warn('Switcher has not been defined at this element');
    }
  }
  mbr.cn = mbr.classname = function (element) {
    return new ClassName(element);
  }
  function Switcher (modifier) {
    this.modifier = modifier;
    this.current = null;
  }
  mbr.classname.switcher = function (modifier) {
    return new Switcher(modifier);
  }
